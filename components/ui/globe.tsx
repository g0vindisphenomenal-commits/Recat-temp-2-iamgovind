...
export function Globe({
  ...
  const globeConfig = useMemo(() => {
    ...
    return {
      ...config,
      dark: isDark ? 1 : 0,
      baseColor: (isDark ? [0.2, 0.2, 0.2] : [0.95, 0.95, 0.95]) as [
        number,
        number,
        number,
      ],
      glowColor: (isDark ? [0.3, 0.3, 0.3] : [0.9, 0.9, 0.9]) as [
        number,
        number,
        number,
      ],
      markerColor: (isDark
        ? [0.2, 0.8, 0.4]
        : [34 / 255, 197 / 255, 94 / 255]) as [number, number, number],
      markers: [
        {
          location: [INDIA.lat, INDIA.lng],
          size: 0.08,
        },
...
const globe = createGlobe(
  ...
  globeOptions as unknown as COBEOptions,
    ...
    const drawOverlay = () => {
      ...
      drawDot(INDIA.lat, INDIA.lng, GREEN);
